...
static int cchInit(tCache * cch, byte * ptr, word dim, byte sec, card ofs) {
    ...
    VERIFY(ptr);
    VERIFY(dim);
    VERIFY(sec != 0xFF);        //no cache for volatile memory
    cch->ptr = ptr;
    cch->dim = dim;
    cch->sec = sec;
    cch->ofs = ofs;
...
static int cchFlush(tCache * cch) {
    ...
    VERIFY(cch->mod + cch->len <= cch->dim);
...
int tabGet(const tTable * tab, word idx, void *buf, word len) {
    ...
    VERIFY(tab);
    VERIFY(buf);
    VERIFY(idx < tabDim(tab));

    if(!len)
        len = tab->wdt + 1;     //assign default value
    VERIFY(tab->wdt <= len);    //is there enough space?

    memset(buf, 0, len);
    if(tab->cch.dim) {
        ofs = (tab->wdt) * idx;
        VERIFY(ofs + tab->wdt <= cchDim(&tab->cch));
...
int recGet(const tRecord * rec, word fld, void *buf, word len) {
    ...
    if(rec->cch.dim) {
        ...
        VERIFY(ofs + rec->len[fld] <= cchDim(&rec->cch));
...
int recPos(const tRecord * rec, const tTable * tab) {
    ...
    VERIFY(tab);
    VERIFY(recSize(rec) == tab->wdt);
    VERIFY(tab->ofs <= rec->ofs);

    ret = (word) ((rec->ofs - tab->ofs) / tabWdt(tab));
    VERIFY(tab->ofs + ret * tabWdt(tab) == rec->ofs);
...
int linFind(tSearch * srh, const char *pat, word beg, word end) {
    ...
    VERIFY(srh);
    VERIFY(pat);
    VERIFY(beg <= tabDim(srh->tab));
    VERIFY(end <= tabDim(srh->tab));
...
static int hshGetIdx(tHash * hsh, word blk, word cell) {    //Get the index value
    ...
    VERIFY(hsh);
    VERIFY(blk <= hsh->dimHsh);
    VERIFY(cell < hsh->dimBlk || cell < hsh->dimOvf);
...
static int hshPutCell(tHash * hsh, word blk, word cell, const byte * key, word idx) {   //Put the (key,idx) cell value
    ...
    VERIFY(key);
    VERIFY(cell < hsh->dimBlk || cell < hsh->dimOvf);
    VERIFY(hsh->len + sizeof(word) <= sizeof(tmp));
...
static int hshLook(const byte * key, byte sec, card ofs, byte len, word dim,
    ...
    cell = 0;
    while(dim) {
        VERIFY(len + sizeof(word) <= sizeof(buf));
