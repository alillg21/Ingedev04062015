  /** \file
 * Unitary UTA test case for integration of GPRS for APR6R/TST4Q/SGM6Y.
 * \sa
 *  - comStart()
 *  - comSet()
 *  - comStop()
 ...
 *
 * $HeadURL: https://svn.ingenico.com/UTA/core/utaYYMMDD/tst/mp/tcmp0005.c $
 *
 * $Id: tcmp0005.c 2534 2010-01-06 10:35:08Z abarantsev $
 */
...
static int testDial(byte ctl) {
    ...
    switch (ctl) {
      case 0:
          //format: primary IP|primary port|secondary IP|secondary port|tertiary IP|tertiary port|
          ptr = "210.1.78.78|60|210.1.78.78|60|210.1.78.78|60|";
          break;
      default:
          break;
    }
...
static int makeMsg(byte ctl, byte * msg) {
    ...
    switch (ctl) {
      case 0:                   
          bufApp(&buf, (byte *) "\x60\x99\x99\x80\x00", 5);  // TPDU
