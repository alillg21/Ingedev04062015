/** \file
 * Unitary test case tcns0019: DEBIT
 * Functions testing:
 * \sa
 */
...
void tcns0019(void) {
    ...
    // AUT with key WK6(type B) to sec 6
    ret = mfcAut(WriteKeyList[6], 0xB6);
    if(ret <= 0)
        goto lblKO;
    pprintf("aut sec 6 ok\n");

    //DECREMENT (DECREASE) VALUE BLOCK with Amount[]
    Amount[0] = 0x01;
    Amount[1] = 0x00;
    Amount[2] = 0x00;
    Amount[3] = 0x00;

    ret = mfcOperation(0x60, MF_DECREMENT, Amount);
    if(ret)
        pprintf("inc ok\n", ret);
    else
        goto lblKO;
    ...
    ret = mfcOperation(0x60, MF_READ, Amount);
    if(ret) {
        pprintf("read ok\n", ret);
