...
int guiMBX(const int type,      // only type==mbTypeCustom is implemented
...
// created              PS      22/04/2005
//===========================================================================
int guiMBMD(const int btnCnt,   // buttons' count (1, 2, 3 allowed)
//      const Pchar * btns,                             // array of button text strings
            const char *btns,   // buttons' text, semicolon (;) separated
            const char *title,  // title (caption) of the messagebox
            const char *msg     // the message to show
...
// created              PS      22/04/2005
//===========================================================================
int guiMBNW(const int btnCnt,   // buttons' count (0, 1, 2, 3 allowed)
//      const Pchar * btns,                             // array of button text strings
            const char *btns,   // buttons' text, semicolon (;) separated
            const char *title,  // title (caption) of the messagebox
            const char *msg     // the message to show
...
// created              PS      22/04/2005
//===========================================================================
int guiMBTO(const int btnCnt,   // buttons' count (0, 1, 2, 3 allowed)
//      const Pchar * btns,                             // array of button text strings
            const char *btns,   // buttons' text, semicolon (;) separated
            const char *title,  // title (caption) of the messagebox
            const char *msg,    // the message to show
...
//===========================================================================
int guiPBPerct(const int style, const char *perct, const char *title,
               const char *msg);
int guiPBMinMax(const int style, const char *min, const char *max,
                const char *value, const char *title, const char *msg);
...
//      uses            : hHmi
//===========================================================================
static int guiEnterNumBase(char *lpBuffer,  // holds the result and can hold initial value
...
// GUI routines
//===========================================================================
static int guiDrawBox(Canvas * pCanv, GC * pGC, int x, int y, int w, int h,
                      int type, int fill);
static int guiDrawBoxTitled(Canvas * pCanv, GC * pGC, int x, int y, int w,
                            int h, int type, int titlesize);
static int guiDrawMsgBox(const int btnCnt, const char *btns, const char *title,
                         const char *msg);
...
// messagebox
//===========================================================================
int guiMBMD(const int btnCnt, const char *btns, const char *title,
            const char *msg) {
    return guiMBX(mbTypeCustom, btnCnt, btns, title, msg, mbIcoNone, -1);
}

int guiMBNW(const int btnCnt, const char *btns, const char *title,
            const char *msg) {
    return guiMBX(mbTypeCustom, btnCnt, btns, title, msg, mbIcoNone, 0);
}

int guiMBTO(const int btnCnt, const char *btns, const char *title,
            const char *msg, const int timeout) {
    return guiMBX(mbTypeCustom, btnCnt, btns, title, msg, mbIcoNone, timeout);
}

int guiMBX(const int type, const int btnCnt, const char *btns,
           const char *title, const char *msg, const int icon,
           const int timeout) {
    ...
    if((hmiOutputConfigRead(hHmi, &config) == RET_OK)
       ...
       && (config.fields & HMI_OUTPUT_CONFIG_WINDOW_GC)) {
        ...
        if(pLocalBitmap) {
            ...
            hmiGraphicDataWrite(hHmi, pLocalBitmap, 0,  // x
                                0,  // y
                                128,    //  
                                64);    // 
        }
    ...
    if((hmiOutputConfigRead(hHmi, &config) == RET_OK)
       ...
       && (config.fields & HMI_OUTPUT_CONFIG_WINDOW_GC)) {
        ...
        if(pLocalBitmap) {
            ...
            guiDrawBox(&localCanvas, pGC, x, y + titlesize - 1, w,
                       y + h - titlesize, type, 0);
...
int guiPBPerct(const int style, const char *perct, const char *title,
    ...
    if((hmiOutputConfigRead(hHmi, &config) == RET_OK)
       ...
       && (config.fields & HMI_OUTPUT_CONFIG_WINDOW_GC)) {
        ...
        hmiADDisplayText(hHmi, 3 | HMI_IN_PIXELS, 4 | HMI_IN_PIXELS,
                         (char *) title);
        ...
        hmiADDisplayText(hHmi, 24 | HMI_IN_PIXELS, 24 | HMI_IN_PIXELS,
                         (char *) msg);
        ...
        if(showvalues) {
            ...
            hmiADFont(hHmi, HMI_INTERNAL_FONT_3);   // Small caraters
            // show text on pos #1
            hmiADDisplayText(hHmi, 50 | HMI_IN_PIXELS,
                             minval + iWidth + 1 | HMI_IN_PIXELS,
                             (char *) perct);
        }
        // reset fg/bg colors
        hmiADSetForegroundColor(hHmi, HMI_COLOR_BLACK);
    ...
    if((hmiOutputConfigRead(hHmi, &config) == RET_OK)
       ...
       && (config.fields & HMI_OUTPUT_CONFIG_WINDOW_GC)) {
        ...
        if(showvalues) {
            // display percentage captions 
            hmiADFont(hHmi, HMI_INTERNAL_FONT_3);   // Small caraters
            hmiADDisplayText(hHmi, 50 | HMI_IN_PIXELS, 5 | HMI_IN_PIXELS,
                             (char *) min);
            hmiADDisplayText(hHmi, 50 | HMI_IN_PIXELS,
                             maxval + 1 | HMI_IN_PIXELS, (char *) max);
            if(iWidth < 30) {
                hmiADDisplayText(hHmi, 50 | HMI_IN_PIXELS,
                                 minval + iWidth + 2 | HMI_IN_PIXELS,
                                 (char *) value);
            } else {
                hmiADSetForegroundColor(hHmi, HMI_COLOR_WHITE);
                hmiADSetBackgroundColor(hHmi, HMI_COLOR_BLACK);
                hmiADDisplayText(hHmi, 50 | HMI_IN_PIXELS,
                                 minval + 3 | HMI_IN_PIXELS, (char *) value);
            }
        }
        // reset fg/bg colors
        hmiADSetForegroundColor(hHmi, HMI_COLOR_BLACK);
...
static int guiDrawMsgBox(const int btnCnt, const char *btns, const char *title,
    ...
    // Display message
    hmiADFont(hHmi, HMI_INTERNAL_FONT_4);   // Thin caraters
    hmiADDisplayText(hHmi, 24 | HMI_IN_PIXELS, 10 | HMI_IN_PIXELS,
                     (char *) msg);
    ...
    if(btnCnt == 1) {
        // show text on pos #1
        hmiADDisplayText(hHmi, 50 | HMI_IN_PIXELS, 46 | HMI_IN_PIXELS,
                         (char *) btns);
    } else if(btnCnt == 2) {
        // show texts on pos #1 and #2
        res = strchr(btns, ch);
        strncpy(frag, btns, res - btns);
        frag[res - btns] = 0;
        hmiADDisplayText(hHmi, 50 | HMI_IN_PIXELS, 49 | HMI_IN_PIXELS, frag);
        strncpy(frag, res + 1, strlen(res + 1));
        frag[strlen(res + 1)] = 0;
        ...
        strncpy(frag, res + 1, res1 - res - 1);
        frag[res1 - res - 1] = 0;
        hmiADDisplayText(hHmi, 50 | HMI_IN_PIXELS, 49 | HMI_IN_PIXELS, frag);
        strncpy(frag, res1 + 1, strlen(res1 + 1));
        frag[strlen(res1 + 1)] = 0;
        hmiADDisplayText(hHmi, 50 | HMI_IN_PIXELS, 90 | HMI_IN_PIXELS, frag);
    }
    // reset fg/bg colors
    hmiADSetForegroundColor(hHmi, HMI_COLOR_BLACK);
