...
static int startGPRSConnection(void){
	...
	if (netStat == 1){
		...
		ret = gsmGprsSetPDPContext(gsmHandle, &gsmGprsContextTIM);
		CHECK(ret==RET_OK,lblReadError);
...
// Not required if you want to do several GPRS without GSM data
	gsmGprsSetPDPActivate (gsmHandle , 0 , ACTIVCID );
	gsmClose(gsmHandle);
}
...
int tcjd0050(int pI1, int pI2, char *pS1, char *pS2) {
