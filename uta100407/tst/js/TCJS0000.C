...
static int ComInit(char bRate)
{
	int ret;
	const byte chn = chnCom1;
	char init[5];
	...
	CHECK(ret >= 0, lblKO);
 	prtS("comStart end");
	//dspLS(0,"Com start End");
	//syswaitforkey();
	
	//prtS("comset");
	
	/******************************************
	USE THIS FUNCTION TO INITIALIZE COM PARAMS
	******************************************/

	prtS("comSet beg");
	memcpy(init+3,&bRate,1);
    ret = comSet(init);
    CHECK(ret >= 0, lblKO);
    prtS("comSet end");
	//dspLS(2,"Comset end");
...
static int Cdma_Connect()
{
	static netScriptCmd_t directClientScriptCmd[]= {
{"AT",   "OK", NULL, 1, 0, 5}, //
{"ATE0", "OK", NULL, 2, 0, 5},
{"AT+crm=1", "OK", NULL, 3, 0, 5},
{"AT$LGPKT=3", "OK", NULL, 4, 0, 5},
{"ATDT#777", "OK", NULL, -1, 0, 60},
{NULL, NULL, NULL, -1, 1, 0} // timeout
};
static netScript_t directClientScript = { directClientScriptCmd, 0};
...
//	int nbElts;
	...
	/* Check only */
	memset(sAt,0,20);
	memcpy(sAt,"ATDT1234\r",9);
	ret = comSendBuf(sAt,9);
	if(ret <= 0) 
	{
		prtS("ret 0");
		return 0;
	}
	prtS("b4 rcv for atdt1234");
	memset(buf,0,sizeof(buf));
	len=2;
	i=0;
	tmrStart(0,30*100);
	...
	memcpy(sAt,"AT$LGPKT=3\r",11);
	prtS(sAt);
	ret = comSendBuf(sAt,11);
	if(ret <= 0) 
	{
		prtS("send fail");
		return 0;
	...
	//nbElts = PAPTBL_NBELTS;
	//pppPapTableConfig(CFG_WRITE_MODE, serverPapTbl, &nbElts);
	memset(&config,0,sizeof(config));
	config.ppp.fields =
	NI_PPP_F_CHANNEL | NI_PPP_F_PPPCFG | NI_PPP_F_SERIALCONFIG | NI_PPP_F_SCRIPTCONFIG;

	config.ppp.channel = i;
	//config.ppp.localIpAddress = inet_addr("111.111.111.111");
	...
	config.ppp.pppCfg.fields =
	NI_PPP_F_LCPFLAGS | NI_PPP_F_IPCPFLAGS | NI_PPP_F_INFLAGS |	NI_PPP_F_LOCALUSERNAME | NI_PPP_F_LOCALPASSWORD ;
	config.ppp.pppCfg.lcpFlags =
	NI_PPP_LCP_PAP | NI_PPP_LCP_MAGIC | NI_PPP_LCP_PCOMP | NI_PPP_LCP_ACOMP;
	config.ppp.pppCfg.ipcpFlags =
	NI_PPP_IPCP_ACCEPT_LOCAL | NI_PPP_IPCP_ACCEPT_REMOTE | NI_PPP_IPCP_DNS1 | NI_PPP_IPCP_DNS2;
	config.ppp.pppCfg.inFlags = NI_PPP_IN_DEFAULT_ROUTE;

	config.ppp.pppCfg.localUsername = "icicipos";
	config.ppp.pppCfg.localPassword = "icicbank";

	config.ppp.scriptConfig = &directClientScript;
	

	ret = netNiConfigSet(niPpp, NET_NI_LEVEL_1, &config);
	CHECK(ret == RET_OK, lblKO);
	prtS("after netNiConfigSet");

	ret = netNiStart(niPpp, 60 * SYS_TIME_SECOND);
	sprintf(buf,"error:%d",ret);
	prtS(buf);
	CHECK(ret == RET_OK, lblKO);
	prtS("after netNiStart");

	ptr= "202.054.135.191-1706"; 
...
/*     INTRODUCTION                                                      */
/*     ============                                                      */
/*     Created :       22-February-2002   L. HURAND                      */
/*     Last modified : 08-January-2004    D .Freeman                     */
/*     Module : LNET PPP demo application                                */
/*     Purpose :                                                         */
/*         Communication Demo. Connects to RAS server in Iran...         */
/*                                                                       */
/*     List of routines in file :                                        */
/*         ComTest : manage the port COM1 +  conect using LNET /GPRS     */
/*                                                                       */
/*     File history :                                                    */
/*     2002-02-22   LHU         Creation                                 */
/*                                                                       */
/*************************************************************************/
...
/*+++******* STATIC TYPES ********************************************---*/
// Naming conventions: xyzSAbcdeFgh_t


/*+++******* STATIC MACROS *******************************************---*/
// Naming convention: XYZs_ABCDE_FGH()
...
// Naming convention: xyz_abcdeFgh : definitions of extern declared in
//                                   xyz.h


/*+++******* INTERNAL GLOBAL DATA DEFINITIONS ************************---*/
// Naming convention: xyzG_abcdeFgh : definitions of extern declared
//                                    in xyzaaaaa.h

/*+++******* STATIC DATA DEFINITIONS *********************************---*/
// Naming convention: xyzS_abcdeFgh : declare as static!
...
/*static char txCMD[30][60]={
		"ATE0X1$QCMDR=3\r" ,   // echo off & simple CONNECT messages
		"atdt#777\r"
};*/
...
//	{"ATDT*99***1#\r",       "CONNECT",      NULL,      -1,    0,     60},  // GPRS 1st PDP context dial string
//	{"ATDT00860922000\r",    "CONNECT",      NULL,      -1,    0,     60},  // Free.fr
//	{"ATDT0,0098216400272\r","CONNECT",      NULL,      -1,    0,     60},  // RAS server in Iran
//	{"ATDT0,0146258170\r",   "CONNECT",      NULL,      -1,    0,     60},  // RAS server via ISDN
//	{"atdt#777\r",           "CONNECT",      NULL,      -1,    0,     60},  // CDMA dial string
...
/*                                                                       */
/*      Input/Output data  :                                             */
/*          This function do not use external data.                      */
/*                                                                       */
/*      Return value :                                                   */
...
/*** STANDARD HMI & PRINTER OPEN ROUTINE FINISHED ***/
    ...
    // Print a nice tittle
	prnPrint(prnHandle, "\x1B\x17\x1B\x1a" "MODEM AT Session");
	...
	uartProtocol.flowControl      = COM_FLOW_HARD; //NONE

    // inter character timeout of 200 * 10 ms = 2 secs
...
//AB			retCode = comSendMsg(comHandle, strlen(txCMD[Counter]), (uint8 *)txCMD[Counter]);
...
//			hmiKeyWait(displayHandle, &key, PSY_INFINITE_TIMEOUT);
   ...
   if (retCode != RET_OK)
   {
      // Failed to close the COM port
   	  // Should display or print the error
   	  return;
   }
...
//AB	config.ppp.channel = i;
	...
	stat = netNiStart(ni, 220*SYS_TIME_SECOND);//120
   ...
   prnLinefeed(prnHandle, 1);
    ...
    // connect to the POP3 server of Free.fr
    //if ((i=tcpClient("212.27.42.12", "110")) < 0) // APAGAR!!!!
...
//AB   destAddr.sin_port = htons((uint16) atoi(szPorta));
   ...
   for (i = 0; i < 8; i++)
   {
	...
	//errorCode = write(hSocket, POP_LOGIN_STR, LEN_POP_LOGIN_STR);
...
/*	errorCode = send(hSocket, POP_PASSWD_STR, LEN_POP_PASSWD_STR, 0);
	...
	if (errorCode == -1)
	{
       ...
       // prnPrint(prnHandle,"Error %d", errorCode);
      //  displayMsg("socket:", "Failed!!!", msgbuf, NULL, 0x0100FFFF);
		prnPrint(prnHandle,"send() socket error");
		return -1;
	}
    else
    {
       prnPrint(prnHandle, "TX : pass mq36noqt");
    }


    memset(rcvData, 0, sizeof(rcvData));
    gRecieveBuffer(hSocket, (int8 *)&rcvData[0], &rcvDataLength);


	errorCode = send(hSocket, POP_STAT_STR, LEN_POP_STAT_STR, 0);
	...
	errorCode = send(hSocket, POP_RETR_STR, LEN_POP_RETR_STR, 0);
...
static long gRecieveBuffer(int16 lSocket,
                           int8  *o_pbyteBuffer,
                           int16 *o_pjub2Size) 
{
   long   lResult = RET_OK;
...
//   char buffer[100];
   ...
   ReadByte = read(lSocket, (void *)&o_pbyteBuffer[0], 1024*4);

   // in this section my buffer size can not be more than 536 
 //  sprintf(buffer,"RX size = %d",ReadByte); 

   if (ReadByte < 64)
   {
      sprintf(buffer, "RX [%d]: %s",ReadByte, &o_pbyteBuffer[0]);
   }
   else
   {
      sprintf(buffer, "RX [%d bytes]",ReadByte);
   }
   prnPrint(prnHandle, buffer); 
