...
static int makeCall(void) {
	...
	srv= "192.1.1.253|1000|255.255.255.0|192.1.1.1|192.1.1.167";
	
	utaGclSetCommCfg(30);//Set the gcl communication timeout
	...
	ret= utaGclRecvBuf(&buf,0,1); 

	ret= dspLS(0,"Receiving done"); CHECK(ret>=0,lblKO);

	ret = dspLS(0, "utaGclStop");
	CHECK(ret >= 0, lblKO);
	utaGclStop();
