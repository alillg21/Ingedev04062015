...
static int prepareCard(byte idx) {
    ...
    CHK;
    ret = mapPut(emvTrmCap, "\x03\xE0\xA8\x80", lenTrmCap + 1);
    CHK;
    ret = mapPut(emvTACDft, "\x05\xD0\x40\x00\xA8\x00", lenTVR + 1);
    CHK;
    /*
       traReset();
...
int CheckChdNam(const char *str){
    ...
    ret= dspLS(2,"ChdNam..."); CHK;
    ret= mapGet(emvChdNam,ChdNam,1+lenChdNam); CHK;
    memset(buf,0,256);
    memcpy(buf,ChdNam+1,*ChdNam);
    ret= prtS(buf+5);
    CHECK(strcmp(buf,str)==0,lblKO);
    ret= dspLS(2,"ChdNam OK"); CHK;
    tmrPause(1);
...
int CheckAppPrfNam(const char *str){
    ...
    ret= dspLS(2,"AppPrfNam..."); CHK;
    ret= mapGet(emvAppPrfNam,AppPrfNam,1+lenAppPrfNam); CHK;
    memset(buf,0,prtW+1);
    memcpy(buf,AppPrfNam+1,*AppPrfNam);
    ret= prtS(buf);
    CHECK(strcmp(buf,str)==0,lblKO);
    ret= dspLS(2,"AppPrfNam OK"); CHK;
    tmrPause(1);
    return 1;
lblKO:
    trcErr(ret);
...
int printTraEmv(void){
    ...
    byte bin[2];
        ...
        ret= prtS(""); CHK;
        ret= prtS("EMV tags"); CHK;

    for(key= traEmvBeg+1; key<traEnd; key++){
                ret= prtS("   --- --- ---"); CHK;
        ret= mapGet(key,buf,1+256); CHK;
        if(*buf==0) continue;
        tag= mapTagKey(key);
                num2bin(bin,tag,2);
        bin2hex(tmp,bin,2);
...
int testCard(byte idx,const char *msg){
    ...
    switch(idx){
        case 1: ret= dspLS(0,"prepare 1..."); break;
        case 2: ret= dspLS(0,"prepare 2..."); break;
        default: VERIFY(idx==1); break; //impossible
    }
    CHECK(ret>=0,lblKO);
    ...
    ret= prepareCard(idx); CHECK(ret>=0,lblKO);
    ret= dspLS(0,msg); CHECK(ret>=0,lblKO);
    ret= enterCard(buf,"c"); CHECK(ret=='c',lblKO);
    ret= dspLS(1,"pmtSmartCard..."); CHECK(ret>=0,lblKO);
    ret= pmtSmartCard(); CHECK(ret>=0,lblKO);
    res= ret;

    ret= printTraEmv(); CHECK(ret>=0,lblKO);
    ret= usrInfo(infRemoveCard); CHECK(ret>=0,lblKO);
    removeCard();
    ret= res;
    goto lblEnd;
lblKO: trcErr(ret); ret= -1;
lblEnd: iccStop(0); return ret;
}
*/
static int lookData(byte idx) {
    ...
    ret = prtS("TSI: ");
    ret = prtS(buf);
    ret = bitTest(tsi + 1, tsiOflDtaAut);
    CHECK(!ret, lblKO);         //Offline Data Authentication was not performed
    ret = dspLS(1, "TSI OK");
    CHK;
    tmrPause(1);
    ...
    ret = prtS("TVR: ");
    ret = prtS(buf);
    ret = bitTest(tvr + 1, tvrOflDtaAut);
    CHECK(ret, lblKO);          //Offline Data Authentication was not performed
    ret = bitTest(tvr + 1, tvrOflSdtAut);
    CHECK(!ret, lblKO);         //Offline Static Data Authentication did not fail
    ret = bitTest(tvr + 1, tvrOflDdtAut);
    CHECK(!ret, lblKO);         //Offline Dinamic Data Authentication did not fail
    /*
       if(idx==2){
