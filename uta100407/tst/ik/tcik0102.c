...
/** NON UTA Unitary test case for GPRS connection with GMA GSM plugin.
 * \sa
 *  - gmaInqGsmGprsInfo()
 *  - gmaInqGsmGprsConn()
 *  - gmaInqGsmGprsDiscon()
...
getGsmGprsInfo(void) {
    ...
    sprintf(buffer, "gmaInqGsmGprsInfo ret: %d", ret);
    prnPrint(prnHandle, buffer);
    sprintf(buffer, "connected: %d", connected);
    prnPrint(prnHandle, buffer);
    sprintf(buffer, "localIpAddr: %d", localIpAddr);
    prnPrint(prnHandle, buffer);
    sprintf(buffer, "dns1: %d", dns1);
    prnPrint(prnHandle, buffer);
    sprintf(buffer, "dns2: %d", dns2);
    prnPrint(prnHandle, buffer);
}
...
void tcik0102(void) {
    ...
    prnPrint(prnHandle, "\x1B\x17\x1B\x1a" "tcik0102");
    ...
    ret = gmaInqGsmGprsConn(&retCode);
    sprintf(buffer, "GsmGprs: %d", retCode);
    prnPrint(prnHandle, buffer);
    if(ret < 0)
        goto lblKO;

    hmiADDisplayText(displayHandle, 0, 0, "getGsmGprsInfo..");
    ret = getGsmGprsInfo();
    if(ret < 0)
        goto lblKO;
    ...
    detach = 1;
    ret = gmaInqGsmGprsDiscon(detach);
    if(ret < 0)
        goto lblKO;
    sprintf(buffer, "GprsDiscon ret: %d", ret);
    prnPrint(prnHandle, buffer);
