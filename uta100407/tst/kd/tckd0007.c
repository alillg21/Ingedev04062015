/** \file
 * Unitary test case tckd0007 for APR.
 * Functions testing:
 * \sa
 *  - comStart()
 *  - comSet()
...
void tckd0007(void) {
    ...
    ret = dspLS(0, "comDial..");
    sprintf( bufPrt , "comSet: %d" , ret ) ;
    prtS( bufPrt ) ;
    tmrPause(1);

    ret = comDial("");
    sprintf( bufPrt , "comDial: %d" , ret ) ;
    prtS( bufPrt ) ;
    CHECK(ret >= 0, lblKODial);

    bufCpy(&buf, (byte *)pMsg, strlen(pMsg) + 1);
