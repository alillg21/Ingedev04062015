 ...
 */
void rsa_initialization( rsa_context *ctx,
               int padding,
               int hash_id )
{
...
signed char a_to_hc (char character)
{
char ch;
...
void ConvertHexStringToData (char *in, char *out, int *length)
{
int				len;
char			data [3];
int				i;
...
for (i=0; i<len; i++)
  {
  sprintf (data,"%.*s",2,(in + i * 2));
  *(out + i) = (char) ha_to_i (data);
  }
...
int genRSAKeyPair(int keyBitSize,int exponent,unsigned char *pubMod,int *pubModLen,unsigned char *privExp,int *privExpLen,unsigned char *pubExp,int *pubExpLen) {
	rsa_context rsa;
	havege_state hs;
	int ret;
	char s[2048];
	...
	rsa_initialization( &rsa, RSA_PKCS_V15, 0 );
	/*memset(&rsa, 0, sizeof( rsa_context ) );
	rsa.padding = RSA_PKCS_V15;
	rsa.hash_id = 0;*/


	if( ( ret = rsa_gen_key( &rsa, havege_rand, &hs, keyBitSize, exponent ) ) != 0 )
	{
		goto exit;
	}
	...
	memset( s, 0, n );
	n -= 2;
	mpi_write_string(&rsa.N,16,s,&n); //Public Modulus
	ConvertHexStringToData(s,pubMod,pubModLen);

	n = sizeof( s );
	memset( s, 0, n );
	n -= 2;
	mpi_write_string(&rsa.D,16,s,&n); //Private Exponent
	ConvertHexStringToData(s,privExp,privExpLen);

	n = sizeof( s );
	memset( s, 0, n );
	n -= 2;
	mpi_write_string(&rsa.E,16,s,&n); //Public Exponent
	ConvertHexStringToData(s,pubExp,pubExpLen);
