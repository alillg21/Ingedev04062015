...
// AESDataIn - it is the array that holds the CipherText to be decrypted.
// AESDataOut - it is the array that holds the output of the for decryption.
// state - the array that holds the intermediate results during decryption.
unsigned char AESDataIn[16], AESDataOut[16];
...
// AESInvCipher is the main function that decrypts the CipherText.
void AESInvCipher()
{
    int i,j,round=0;
    ...
    for(i=0;i<4;i++)
    {
        for(j=0;j<4;j++)
        {
            AESDataOut[i*4+j]=state[j][i];
        }
    }
}

// Tj.H
// AESCipher is the main function that encrypts the PlainText.
void AESCipher (void)
{
    int i,j,round=0;
...
int AESLoadKey (unsigned char *key, int key_length)
{
    ...
    Nr = Nk + 6;
    //
    memmove (AESKey, key, key_length / 8);
    AESKeyExpansion ();
    return 1;
}
...
void AESEncrypt (unsigned char *data_in, unsigned int data_in_len, unsigned char *data_out)
{
    ...
    {
        memmove (AESDataIn, data_in + i * 16, 16);
        AESCipher ();
        memmove (data_out + i * 16, AESDataOut, 16);
    }
}
...
//     Part 1: ********************************************************
    ...
    */
    ...
    memmove (AESKey, "\x95\xA8\xEE\x8E\x89\x97\x9B\x9E\xFD\xCB\xC6\xEB\x97\x97\x52\x8D\x43\x2D\xC2\x60\x61\x55\x38\x18\xEA\x63\x5E\xC5\xD5\xA7\x72\x7E", 32);
    memmove (AESDataIn, "\x4E\xC1\x37\xA4\x26\xDA\xBF\x8A\xA0\xBE\xB8\xBC\x0C\x2B\x89\xD6", 16);
...
//             ********************************************************
    ...
    // The AESKeyExpansion routine must be called before encryption.
    AESKeyExpansion();

    // The next function call encrypts the PlainText with the Key using AES algorithm.
    AESCipher ();

    memmove (AESDataIn, AESDataOut, 16);
